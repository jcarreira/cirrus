...
bundle_zip_location = "/home/rnithin/joao/" \
        + "python/frontend/cirrus/cirrus/bundle.zip"
...
#        print ips
    ...
    # User must either specify param_dict_lst
    # or hyper_vars, hyper_params, and param_base
    def set_task_parameters(self, task, param_base=None, \
            hyper_vars=None, hyper_params=None, machines=None):
        ...
        for p in possibilities:
            ...
            if not lambda_exists(lambdas, lambda_name):
    ...
    def get_cost_per_sec(self):
        return sum([c.cost_model.get_cost_per_second() \
    ...
    def start_queue_threads(self):
        ...
        def custodian(cirrus_objs, thread_id, num_jobs):
            index = thread_id
            msg = "Custodian number %d starting..." % thread_id
            logging.info(msg)
        ...
        def copy_and_run(thread_id):
            while True:
                ...
                os.system(cmd)
                cmd = 'ssh %s "killall parameter_server; " \
                        + "chmod +x %s; ./%s &"' \
                        % (ubuntu_machine, sh_file, sh_file)
                os.system(cmd)
        ...
        for i in range(self.num_jobs):
            p = threading.Thread(target=custodian, \
                    args=(self.cirrus_objs, i, self.num_jobs))
            p.start()

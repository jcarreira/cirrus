...
class Ec2VMManager(object):
    ...
    def start_vm_spot(self, number_vms, key_name, price="0.01"):
        assert(number_vms == 1)
        print "Starting Spot Instance Request for %d VMs at price %s" \
                % (number_vms, price)
        ...
        while state == 'open':
            ...
            time.sleep(10)
            spot = client.describe_spot_instance_requests(DryRun=False,\
        ...
        tags = [{'Key':'runtime', 'Value': 'Cirrus 0.1'}, \
                {'Key':'owner', 'Value': 'Cirrus'}]
    ...
    def stop_all_vms(self):
        ...
        for instance_id in self.instances:
            # instance = self.ec2_resource.instances.filter( \
            # InstanceIds=[instance_id.instance_id]).stop()
            response = self.ec2_resource.instances \
                    .filter(InstanceIds=[instance_id.instance_id]).terminate()
            print response
    ...
    def list_vm_instances_with_tag(self):
        for instance in self.ec2_resource.instances.all():

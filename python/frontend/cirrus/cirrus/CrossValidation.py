...
class CrossValidation:
    ...
    def __init__(self, task=None, param_base=None, num_sets=10, machines=[], num_jobs=1, timeout=-1):
    ...
    def set_task_parameters(self, task, num_sets, param_base=None, machines=[]):
        base_port = 1337
        index = 0
        ...
        for i in range(10):
            ...
            modified_config['test_set'] = sets[i]
    ...
    def get_info_for(self, i):
        string = ""
        string += "%s: %s\n" % ('train_set', str(self.param_lst[i]['train_set']))
        return string
    ...
    def start_queue_threads(self):
        ...
        def custodian(cirrus_objs, thread_id, num_jobs):
            ...
            while True:
                ...
                if repeats % 5 == 0:
                    time_to_loss = {}
                    for x in self.loss_lst:
                        for loss in x:
                            rounded_time = int(round(loss[0]))
                            if rounded_time in time_to_loss:
                                time_to_loss[rounded_time].append(loss[1])
                            else:
                                time_to_loss[rounded_time] = [loss[1]]
                    loss_list = []

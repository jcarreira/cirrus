...
void MFModel::initialize_data(uint64_t users, uint64_t items,
  ...
  //    new FEATURE_TYPE[items * nfactors],
  // std::default_delete<FEATURE_TYPE[]>());
...
std::unique_ptr<CirrusModel> MFModel::deserialize(void* /*data*/,
                                                  uint64_t /*size*/) const {
  throw std::runtime_error("Not implemented");
}

std::pair<std::unique_ptr<char[]>, uint64_t> MFModel::serialize() const {
...
void MFModel::serializeTo(void* mem) const {
  ...
  std::cout << "SerializeTo"
            << " nusers: " << nusers_ << " nitems_: " << nitems_
            << " nfactors_: " << nfactors_ << std::endl;
#endif
...
void MFModel::sgd_update(double learning_rate, const ModelGradient* gradient) {
  const MFSparseGradient* grad_ptr =
      dynamic_cast<const MFSparseGradient*>(gradient);
  assert(grad_ptr);
...
#ifdef DEBUG
  FEATURE_TYPE res =
      global_bias_ + user_bias_.at(userId) + item_bias_.at(itemId);
#else
...
const FEATURE_TYPE& MFModel::get_user_weights(uint64_t userId,
                                              uint64_t factor) const {
  return user_weights_.at(userId * nfactors_ + factor);
}

const FEATURE_TYPE& MFModel::get_item_weights(uint64_t itemId,
                                              uint64_t factor) const {
  return item_weights_.at(itemId * nfactors_ + factor);
}
    ...
    for (uint64_t j = 0; j < dataset.data_[i].size(); ++j) {
...
#ifdef DEBUG
      std::cout << "user: " << user << "itemId: " << itemId
                << "rating: " << rating << " prediction: " << pred
                << " error: " << error << std::endl;
#endif

      if (itemId >= nitems_ || user >= nusers_) {
        std::cout << "itemId: " << itemId << " nitems_: " << nitems_
                  << " user: " << user << " nusers_: " << nusers_ << std::endl;
        throw std::runtime_error("Wrong value here");
      }
      user_bias_[user] +=
          learning_rate * (error - user_bias_reg_ * user_bias_[user]);
      item_bias_[itemId] +=
          learning_rate * (error - item_bias_reg_ * item_bias_[itemId]);
      ...
      for (uint64_t k = 0; k < nfactors_; ++k) {
        FEATURE_TYPE delta_user_w =
            learning_rate * (error * get_item_weights(itemId, k) -
                             user_fact_reg_ * get_user_weights(user, k));
        // std::cout << "delta_user_w: " << delta_user_w << std::endl;
        get_user_weights(user, k) += delta_user_w;
#ifdef DEBUG
        if (std::isnan(get_user_weights(user, k)) ||
            std::isinf(get_user_weights(user, k))) {
      ...
      for (uint64_t k = 0; k < nfactors_; ++k) {
        FEATURE_TYPE delta_item_w =
            learning_rate * (error * get_user_weights(user, k) -
        ...
        if (std::isnan(get_item_weights(itemId, k)) ||
          ...
          std::cout << "user weight: " << get_user_weights(user, k)
                    << std::endl;
...
std::pair<double, double> MFModel::calc_loss(SparseDataset& dataset,
...
double MFModel::checksum() const {
  return crc32(user_weights_.data(),
               user_weights_.size() * sizeof(FEATURE_TYPE));
}

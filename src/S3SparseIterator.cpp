...
S3SparseIterator::S3SparseIterator(uint64_t left_id,
  ...
  thread =
      new std::thread(std::bind(&S3SparseIterator::thread_function, this, c));
...
const void* S3SparseIterator::get_next_fast() {
  ...
  if (to_delete != -1) {
#ifdef DEBUG
    std::cout << "get_next_fast::Deleting entry: " << to_delete << std::endl;
#endif
    list_strings.erase(to_delete);
#ifdef DEBUG
    std::cout << "get_next_fast::Deleted entry: " << to_delete << std::endl;
#endif
  }
...
void S3SparseIterator::push_samples(std::ostringstream* oss) {
  ...
  (void) num_samples;
#ifdef DEBUG
  std::cout << "push_samples s3_obj_size: " << s3_obj_size
...
uint64_t S3SparseIterator::get_obj_id(uint64_t left, uint64_t right) {
  if (random_access) {
    ...
    std::cout << "Sampled : " << sampled << " worker_id: " << worker_id
...
void S3SparseIterator::print_progress(const std::string& s3_obj) {
  ...
  std::cout << "Getting object count: " << count << " s3 e2e bw (MB/s): "
            << total_received / elapsed_sec / 1024.0 / 1024 << std::endl;
}
...
void S3SparseIterator::thread_function(const Configuration& config) {
  ...
  while (1) {
    ...
    if (config.get_s3_bucket() == "cirrus-criteo-kaggle-19b-random" ||
      ...
      obj_id_str =
          std::to_string(hash_f(std::to_string(obj_id).c_str())) + "-CRITEO";
    } else {
  ...
  try_start:
    try {
      std::cout << "S3SparseIterator: getting object " << obj_id_str
                << std::endl;
      uint64_t start = get_time_us();
      s3_obj = s3_client->s3_get_object_ptr(obj_id_str, config.get_s3_bucket());
      uint64_t elapsed_us = (get_time_us() - start);
      double mb_s =
          sstream_size(*s3_obj) / elapsed_us * 1000.0 * 1000 / 1024 / 1024;
      std::cout << "received s3 obj"
                << " elapsed: " << elapsed_us
                << " size: " << sstream_size(*s3_obj) << " BW (MB/s): " << mb_s
                << "\n";
// pm.increment_batches(); // increment number of batches we have processed

#ifdef DEBUG
// double MBps = (1.0 * (32812.5*1024.0) / elapsed_us) / 1024 / 1024 * 1000 *
// 1000;
...
//  << std::endl;
    ...
    // std::cout << "pushing took (us): " << elapsed_us << " at (us) " <<
    // get_time_us() << std::endl;
  }
}

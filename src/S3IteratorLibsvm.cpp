...
std::shared_ptr<SparseDataset> S3IteratorLibsvm::getNext() {
   ...
   */
  bool S3IteratorLibsvm::ignoreSpaces(uint64_t& index,
...
#ifdef DEBUG
        std::cout << "Finished text returning " << result.size()
                  << " minibatches" << std::endl;
#endif
        return result;
      }
  ...
  std::pair<uint64_t, uint64_t> S3IteratorLibsvm::getFileRange(
    ...
    if (random_access) {
      ...
      auto ret = std::make_pair(cur_index,
                                std::min(cur_index + FETCH_SIZE, file_size));

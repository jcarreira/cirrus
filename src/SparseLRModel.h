...
class SparseLRModel : public CirrusModel {
 public:
   ...
   * @param gradient Gradient to be used for the update
   */
  void sgd_update_adagrad(double learning_rate, const ModelGradient* gradient);
  ...
  void sgd_update_momentum(double learning_rate,
   ...
   */
  std::unique_ptr<ModelGradient> minibatch_grad(const SparseDataset& dataset,
  ...
  std::vector<FEATURE_TYPE> get_weight_history() { return weights_hist_; }

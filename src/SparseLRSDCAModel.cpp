...
namespace cirrus {
    ...
    SparseLRSDCAModel::SparseLRSDCAModel(uint64_t d) {
        weights_.resize(d);
        weights_hist_.resize(d);
    }

    SparseLRSDCAModel::SparseLRSDCAModel(const FEATURE_TYPE* w, uint64_t d) {
        weights_.resize(d);
        weights_hist_.resize(d);
        std::copy(w, w + d, weights_.begin());
    }

    uint64_t SparseLRSDCAModel::size() const {
...
/** FORMAT
  ...
  */
    std::unique_ptr<CirrusModel> SparseLRSDCAModel::deserialize(void* data, uint64_t size) const {
        ...
        std::unique_ptr<SparseLRSDCAModel> model = std::make_unique<SparseLRSDCAModel>(
   ...
   */
    ...
    std::unique_ptr<CirrusModel> SparseLRSDCAModel::copy() const {
        std::unique_ptr<SparseLRSDCAModel> new_model =
                std::make_unique<SparseLRSDCAModel>(weights_.data(), size());
        return new_model;
    }

    void SparseLRSDCAModel::sgd_update_adagrad(double learning_rate,
    ...
    void SparseLRSDCAModel::sgd_update_momentum(double learning_rate, double momentum_beta,
                                            const ModelGradient* gradient) {
        const LRSparseGradient* grad =
                dynamic_cast<const LRSparseGradient*>(gradient);
    ...
    std::unique_ptr<ModelGradient> SparseLRSDCAModel::minibatch_grad(
...
#define CROSS_ENTROPY_LOSS
    ...
    std::unique_ptr<ModelGradient> SparseLRSDCAModel::minibatch_grad_sparse(
            const SparseDataset& dataset,
            const Configuration& config) const {

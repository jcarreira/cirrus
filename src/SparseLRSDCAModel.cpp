...
SparseLRSDCAModel::SparseLRSDCAModel(const FEATURE_TYPE *w, uint64_t d, const FEATURE_TYPE *a, uint64_t n) {
  weights_.resize(d);
...
/** FORMAT
  * weights
  */
std::unique_ptr<CirrusModel> SparseLRSDCAModel::deserialize(void *data, uint64_t size) const {
  throw std::runtime_error("not supported");
}

std::pair<std::unique_ptr<char[]>, uint64_t>
SparseLRSDCAModel::serialize() const {
...
void SparseLRSDCAModel::serializeTo(void *mem) const {
  ...
  std::copy(coord_weights_.data(), coord_weights_.data() + coord_weights_.size(),
            reinterpret_cast<FEATURE_TYPE *>(mem));
}

uint64_t SparseLRSDCAModel::getSerializedSize() const {
  auto ret = (primal_size() + dual_size()) * sizeof(FEATURE_TYPE) + 2 * sizeof(int);
  return ret;
}
...
std::unique_ptr<CirrusModel> SparseLRSDCAModel::copy() const {
  std::unique_ptr<SparseLRSDCAModel> new_model =
      std::make_unique<SparseLRSDCAModel>(weights_.data(), primal_size(), coord_weights_.data(), dual_size());
  return new_model;
}
...
void SparseLRSDCAModel::sgd_update(double learning_rate,
  ...
  // TODO: Find correct scaling for updates due to minibatching
...
double SparseLRSDCAModel::norm_squared(const std::vector<std::pair<int, FEATURE_TYPE>> &v1) const {
  auto total = 0.0;
...
auto start = get_time_us();
  ...
  for (int i = 0; i < dataset.num_samples(); i += 1) {
    ...
    FEATURE_TYPE numerator = 1.0 + std::exp(dot_product(x, weights_) * dataset.labels_[i]);
    numerator = (dataset.labels_[i] / numerator) - coord_weights_[i + starting_index];

    FEATURE_TYPE denominator = std::max(1.0,
                                        0.25 + (norm_squared(x) / (learning_rate * dual_size())));
  ...
  std::unique_ptr<LRSDCASparseGradient> ret =
      std::make_unique<LRSDCASparseGradient>(std::move(w_grad), std::move(a_grad));
#ifdef DEBUG
...
std::pair<double, double> SparseLRSDCAModel::calc_loss(SparseDataset &dataset, uint32_t) const {
  double total_loss = 0;
  auto w = weights_;
...
std::unique_ptr<ModelGradient> SparseLRSDCAModel::loadGradient(void *mem) const {
  throw std::runtime_error("Not supported");
}
...
void SparseLRSDCAModel::ensure_preallocated_vectors(const Configuration &config) const {

...
void PerformanceLambdaTask::run(const Configuration& config) {
  ...
  S3Iterator s3_iter(0, num_s3_batches, config, config.get_s3_size(),
                     features_per_sample, config.get_minibatch_size(),
  ...
  while (1) {
    ...
    gradient = lr_model->minibatch_grad(
        dataset.samples_, const_cast<FEATURE_TYPE*>(dataset.labels_.get()),
        samples_per_batch, config.get_epsilon());

...
// Row major order
SoftmaxModel::SoftmaxModel(const FEATURE_TYPE* data,
                           uint64_t nclasses,
                           uint64_t d)
...
SoftmaxModel::SoftmaxModel(std::vector<std::vector<FEATURE_TYPE>> data,
                           uint64_t nclasses,
                           uint64_t d)
    : nclasses(nclasses), d(d) {
  weights.resize(d);
...
/** Serialization Format:
  ...
  */
std::pair<std::unique_ptr<char[]>, uint64_t> SoftmaxModel::serialize() const {
...
std::unique_ptr<CirrusModel> SoftmaxModel::deserialize(
    void* data,
    uint64_t /* size */) const {
...
void SoftmaxModel::sgd_update(double learning_rate,
                              const ModelGradient* gradient) {
  const SoftmaxGradient* grad = dynamic_cast<const SoftmaxGradient*>(gradient);
...
std::unique_ptr<ModelGradient> SoftmaxModel::minibatch_grad(
  ...
  const FEATURE_TYPE* m_data =
      reinterpret_cast<const FEATURE_TYPE*>(m.data.get());
  ...
  // [N * K]
  auto probs = exp_scores.unaryExpr(
      [exp_scores_sum](FEATURE_TYPE v) { return v / exp_scores_sum; });
...
std::pair<double, double> SoftmaxModel::calc_loss(Dataset& data) const {
  ...
  for (unsigned int i = 0; i < dataset.rows(); ++i) {
    FEATURE_TYPE class_i =
        reinterpret_cast<const FEATURE_TYPE*>(data.labels_.get())[i];
    ...
    if (std::isnan(logprobs[i]) || std::isinf(logprobs[i])) {
      std::cerr << probs(i, class_i) << " " << class_i << std::endl;
      throw std::runtime_error("Invalid logprob");
    }
    sum += logprobs[i];
  }
  ...
  std::cout << "Accuracy: " << accuracy << " wrong: " << count_wrong
            << " samples: " << dataset.rows() << std::endl;

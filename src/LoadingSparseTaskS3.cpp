...
SparseDataset LoadingSparseTaskS3::read_dataset(
  ...
  return input.read_input_criteo_kaggle_sparse(config.get_input_path(), delimiter, config);
}
...
void LoadingSparseTaskS3::check_loading(const Configuration& config,
  ...
  std::string obj_id = std::to_string(hash_f(std::to_string(SAMPLE_BASE).c_str())) + "-MNIST";
  std::string data =
...
void LoadingSparseTaskS3::run(const Configuration& config) {
  ...
  for (unsigned int i = 0; i < num_s3_objs; ++i) {
    ...
    std::string obj_id = std::to_string(hash_f(std::to_string(SAMPLE_BASE + i).c_str())) + "-MNIST";

...
void PSSparseServerInterface::send_lr_sdca_gradient(
    const LRSDCASparseGradient& gradient) {
...
std::unique_ptr<CirrusModel> PSSparseServerInterface::get_full_model(
  ...
  if (isCollaborative) {
    ...
    if (read_all(sock, model_data_ptr, model_size * sizeof(FEATURE_TYPE)) ==
        0) {
      throw std::runtime_error("Error talking to PS");
    }
...
void PSSparseServerInterface::get_lr_sdca_model_inplace(
...
SparseLRSDCAModel PSSparseServerInterface::get_lr_sdca_model(
    const Configuration& config) {

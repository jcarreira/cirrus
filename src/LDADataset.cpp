...
namespace cirrus{
  ...
  LDADataset::LDADataset(const char* msg_begin){
    ...
    msg_begin = reinterpret_cast<const char*>(reinterpret_cast<const char*>(msg_begin) +  sizeof(int));
    ...
    for(int i=0; i<*V; ++i){
      ...
      msg_begin = reinterpret_cast<const char*>(reinterpret_cast<const char*>(msg_begin) +  *len);
    }
  ...
  char* LDADataset::serialize(){
    ...
    serialize_size = (2 * N + D + V_word + 2) * sizeof(int) + V_letter * sizeof(char);
    return msg_begin;

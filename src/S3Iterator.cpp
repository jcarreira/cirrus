...
S3Iterator::S3Iterator(uint64_t left_id,
                       uint64_t right_id,
...
void S3Iterator::push_samples(std::ostringstream* oss) {
  ...
  for (uint64_t i = 0; i < n_minibatches; ++i) {
    const FEATURE_TYPE* data =
        reinterpret_cast<const FEATURE_TYPE*>(str_iter->second.c_str()) +
        i * minibatch_n_entries;
...
void S3Iterator::thread_function() {
  std::cout << "Building S3 deser. with size: " << s3_rows << " x "
            << (s3_cols + 1) << " = " << (s3_rows * (s3_cols + 1)) << std::endl;
  ...
  try_start:
    try {
      ...
      double MBps = (1.0 * (32812.5 * 1024.0) / elapsed_ns) / 1024 / 1024 *
                    1000 * 1000 * 1000;
      std::cout << "Get s3 obj took (us): " << (elapsed_ns / 1000.0)
                << " size (KB): " << 32812.5 << " bandwidth (MB/s): " << MBps
                << std::endl;
    } catch (...) {
    ...
    uint64_t elapsed_ns2 =
        std::chrono::duration_cast<std::chrono::nanoseconds>(finish2 - start)
            .count();
    std::cout << "pushing took (ns): " << elapsed_ns2 << " at (us) "

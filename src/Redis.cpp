...
void redis_put(redisContext* c, const char* id, const char* s) {
#ifdef REDIS_DEBUG
  std::cout << "Redis put"
            << " id: " << id << " id size: " << strlen(id) << std::endl;
#endif
  redisReply* reply = (redisReply*) redisCommand(c, "SET %s %s", id, s);

#ifdef REDIS_DEBUG
  std::cout << "Redis put"
            << " reply type: " << reply->type << " status: " << reply->str
            << std::endl;
#endif
...
void redis_put_binary(redisContext* c,
                      const char* id,
                      const char* s,
                      size_t size) {
#ifdef REDIS_DEBUG
  std::cout << "Redis put binary"
            << " id: " << id << " data size: " << size << std::endl;
#endif
  redisReply* reply = (redisReply*) redisCommand(c, "SET %s %b", id, s, size);

#ifdef REDIS_DEBUG
  std::cout << "Redis put binary"
            << " reply type: " << reply->type << " status: " << reply->str
            << " size: " << reply->len << std::endl;
#endif

  freeReplyObject(reply);
}

void redis_put_binary_numid(redisContext* c,
                            uint64_t id,
                            const char* s,
                            size_t size) {
#ifdef REDIS_DEBUG
...
#endif
  redisReply* reply =
      (redisReply*) redisCommand(c, "GET %b", id, (size_t) strlen(id));
...
void redis_delete(redisContext* c, const char* id) {
  redisReply* reply = (redisReply*) redisCommand(c, "DEL %s", id);
  freeReplyObject(reply);
}
...
void redis_subscribe_callback(redisAsyncContext* c,
                              sub_handler h,
                              const char* name) {

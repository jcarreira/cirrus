...
/** FORMAT
  * weights
  */
std::unique_ptr<CirrusModel> SparseLRModel::deserialize(void* data,
                                                        uint64_t size) const {
  throw std::runtime_error("not supported");
  uint64_t d = size / sizeof(FEATURE_TYPE);
  std::unique_ptr<SparseLRModel> model =
      std::make_unique<SparseLRModel>(reinterpret_cast<FEATURE_TYPE*>(data), d);
  return model;
}

std::pair<std::unique_ptr<char[]>, uint64_t> SparseLRModel::serialize() const {
...
void SparseLRModel::sgd_update_adagrad(double learning_rate,
  ...
  for (const auto& w : grad->weights) {
    ...
    weights_[index] +=
        learning_rate * value / (adagrad_epsilon + std::sqrt(weight_hist));
  }
}

void SparseLRModel::sgd_update_momentum(double learning_rate,
...
void SparseLRModel::loadSerializedSparse(const FEATURE_TYPE* weights,
  ...
  assert(num_weights > 0 && num_weights < 10000000);
  std::cout << "Loading weights: " << num_weights << std::endl;
...
void SparseLRModel::ensure_preallocated_vectors(
...
std::unique_ptr<ModelGradient> SparseLRModel::minibatch_grad_sparse(
  ...
  for (uint64_t i = 0; i < dataset.num_samples(); ++i) {
    ...
    for (const auto& feat : dataset.get_row(i)) {
      ...
      part1_i +=
          value *
          weights_sparse_[index];  // 25% of the execution time is spent here
    }
    part2[i] = dataset.labels_[i] - s_1(part1_i);
  }
  ...
  for (auto& v : unique_indices) {
    ...
    // we set this to 0 so that next iteration part3 is all 0s
      ...
      part3[index] = 0;
    double final_grad =
        value + weights_sparse_[index] * 2 * config.get_epsilon();

...
/** FORMAT
  * weights
  */
std::unique_ptr<CirrusModel> LRModel::deserialize(void* data,
                                                  uint64_t size) const {
  uint64_t d = size / sizeof(FEATURE_TYPE);
  std::unique_ptr<LRModel> model =
      std::make_unique<LRModel>(reinterpret_cast<FEATURE_TYPE*>(data), d);
  return model;
}

std::pair<std::unique_ptr<char[]>, uint64_t> LRModel::serialize() const {
...
void LRModel::sgd_update(double learning_rate, const ModelGradient* gradient) {
...
std::unique_ptr<ModelGradient> LRModel::minibatch_grad(const Matrix& dataset,
                                                       FEATURE_TYPE* labels,
  ...
  Eigen::Map<Eigen::Matrix<FEATURE_TYPE, Eigen::Dynamic, Eigen::Dynamic,
                           Eigen::RowMajor>>
...
std::pair<double, double> LRModel::calc_loss(Dataset& dataset) const {
  ...
  Eigen::Map<Eigen::Matrix<FEATURE_TYPE, Eigen::Dynamic, Eigen::Dynamic,
                           Eigen::RowMajor>>
  ds(const_cast<FEATURE_TYPE*>(ds_data), dataset.samples_.rows,
     dataset.samples_.cols);

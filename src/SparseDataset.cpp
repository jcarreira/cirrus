  ...
  max_features_ =
      std::max_element(data_.begin(), data_.end(),
                       [](const std::vector<std::pair<int, FEATURE_TYPE>>& a,
                          const std::vector<std::pair<int, FEATURE_TYPE>>& b) {
...
SparseDataset::SparseDataset(const char* data,
                             uint64_t n_samples,
                             bool has_labels) {
...
SparseDataset::SparseDataset(const char* data, bool from_s3, bool has_labels) {
  ...
  std::cout << "SparseDataset constructor"
            << " from_s3: " << from_s3 << " obj_size: " << obj_size
  ...
  for (const auto& w : data_) {
    for (const auto& v : w) {
      ...
      if (std::isnan(rating) || std::isinf(rating)) {
        throw std::runtime_error("SparseDataset::check_values nan/inf rating");
      }
    }
  }
}
    ...
    for (const auto& v : w) {
      ...
      if (rating < -100 || rating > 100) {
        throw std::runtime_error(
            "SparseDataset::check_values wrong rating value: " +
...
void SparseDataset::check_labels() const {
  for (const auto& l : labels_) {
    if (std::isnan(l) || std::isinf(l)) {
      throw std::runtime_error("SparseDataset::check_labels nan/inf rating");
    }
    if (l != 0.0 && l != 1.0) {
...
/** FORMAT OF S3 object
  ...
  * Sample 1: Label (FEATURE_TYPE) | number of values (int) | index1 (int) |
 * value1 (FEATURE_TYPE) | index2 | ...
  * Sample 2: ...
  * ------------- Without labels (store_labels = false)
  * Sample 1: number of values (int) | index1 (int) | value1 (FEATURE_TYPE) |
 * index2 | ...
  * Sample 2: ...
  */
std::shared_ptr<char> SparseDataset::build_serialized_s3_obj(
  ...
  *obj_size +=
      n_samples * ((store_labels ? sizeof(FEATURE_TYPE) : 0) + sizeof(int));
  *obj_size +=
      sizeof(int) *
      2;  // we also store the size of the obejct and the number of samples

  // allocate memory for this object
  std::shared_ptr<char> s3_obj =
      std::shared_ptr<char>(new char[*obj_size], std::default_delete<char[]>());
...
SparseDataset SparseDataset::sample_from(uint64_t start,
                                         uint64_t n_samples) const {
...
void SparseDataset::normalize(uint64_t hash_size) {
  ...
  std::vector<FEATURE_TYPE> min_val_feature(
      hash_size, std::numeric_limits<FEATURE_TYPE>::max());
...
const std::vector<std::pair<int, FEATURE_TYPE>>& SparseDataset::get_row(
    uint64_t n) const {
  if (n >= data_.size()) {

...
class ModelGradient {
 public:
   ...
   */
  virtual void setVersion(uint64_t v) { version = v; }

  /**
   * Get version of gradient
   */
  virtual uint64_t getVersion() const { return version; }
...
class SoftmaxGradient : public ModelGradient {
 ...
 protected:
  ...
  std::vector<std::vector<FEATURE_TYPE>>
      weights;  //< weights for softmax gradient
};
...
class MFSparseGradient : public ModelGradient {
 public:
  ...
  void print() const {
    std::cout << users_bias_grad.size() << " / " << users_weights_grad.size()
              << std::endl;
    std::cout << items_bias_grad.size() << " / " << items_weights_grad.size()
              << std::endl;
  }
  void check_values() const override;

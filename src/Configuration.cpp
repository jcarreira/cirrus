...
Configuration::Configuration() : learning_rate(-1), epsilon(-1) {}

Configuration::Configuration(const std::string& path)
    : learning_rate(-1), epsilon(-1) {
  read(path);
}
...
void Configuration::print() const {
  ...
  std::cout << "train_set: " << train_set_range.first << "-"
            << train_set_range.second << std::endl;
  std::cout << "test_set: " << test_set_range.first << "-"
            << test_set_range.second << std::endl;
...
void Configuration::parse_line(const std::string& line) {
  ...
  if (iss.fail() && s.find_first_not_of("\t\n\v\f\r") !=
